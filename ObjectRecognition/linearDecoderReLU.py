...
def costfunc_gpu_ReLU(x, *args):
    ...
    cost = gpu.sum(output_target_diff)/(2*nData) + 0.5 * lambda_val * (gpu.sum(regularized_penalty1) + gpu.sum(regularized_penalty2))
...
def grad_costfunc_gpu_ReLU(x, *args):
    ...
    hidden_activation = gpu.concatenate((gpu.ones((1,nData)), hidden_activation), axis = 0)
    hidden_derivative = gpu.concatenate((gpu.ones((1,nData)), hidden_derivative), axis = 0)
    outputs = gpu.dot(weights2, hidden_activation)
    weights1_grad = gpu.zeros(shape(weights1))
    weights2_grad = gpu.zeros(shape(weights2))
    p = outputs-inputs
...
def checkGradient():
    ...
    for p in range(size(weights)):
        ...
        loss1 = costfunc_ReLU(minus_weights, *args)
        loss2 = costfunc_ReLU(plus_weights, *args)
        numgrad[p] = (loss2 - loss1) / (2*e)
        perturb[p] = 0
    grad = grad_costfunc_ReLU(weights, *args)
    print numgrad
    print grad
...
def linear_decoder_run_gpu(data, weights, numInput, numHidden):
    ...
    beta = 3.5
    inputs = data
    args = (num_input,num_hidden,num_output,inputs,data,lambda_val,sparsityParam,beta)
    ...
    scipy.io.savemat('learntNoisyFeaturesGPU.mat', mdict={'learntFeatures': weights1})
    return weights
    
def linear_decoder_run_gpu_noCNN(data, weights, numInput, numHidden):
    print "Starting Feature Abstraction..."
    num_input = numInput
    num_hidden = numHidden
    num_output = numInput
    ...
    sparsityParam = 0.035
    beta = 5
    args = (num_input,num_hidden,num_output,data,data,lambda_val,sparsityParam,beta)
    opttheta = optimize.fmin_l_bfgs_b(costfunc_gpu, weights, fprime=grad_costfunc_gpu, args=args, maxiter=400)
    weights = opttheta[0]
    weights1 = reshape(weights[0:num_weights1],(num_hidden,num_input+1))
    #weights2 = reshape(weights[num_weights1:shape(weights)[0]], (num_output,num_hidden+1))
...
def linear_decoder_run_ReLU(data, numInput, numHidden):
    ...
    inputs = data
    args = (num_input,num_hidden,num_output,inputs,lambda_val,sparsityParam,beta)
    opttheta = optimize.fmin_l_bfgs_b(costfunc_gpu_ReLU, weights, fprime=grad_costfunc_gpu_ReLU, args=args, maxiter=400)
    weights = opttheta[0]
    weights1 = reshape(weights[0:num_weights1],(num_hidden,num_input+1))
...
def linear_decoder_run(data, numInput, numHidden):
    ...
    weights = hstack((weights1,weights2))
    args = (num_input,num_hidden,num_output,inputs,lambda_val,sparsityParam,beta)
    opttheta = optimize.fmin_l_bfgs_b(costfunc, weights, fprime=grad_costfunc, args=args, maxiter=500)
    weights = opttheta[0]
    weights1 = reshape(weights[0:num_weights1],(num_hidden,num_input+1))
    weights2 = reshape(weights[num_weights1:shape(weights)[0]], (num_output,num_hidden+1))
